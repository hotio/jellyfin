...
# install app
RUN debfile="/tmp/jellyfin.deb" && curl -fsSL -o "${debfile}" "https://repo.jellyfin.org/releases/server/ubuntu/stable/server/jellyfin-server_${JELLYFIN_VERSION}_amd64.deb" && dpkg --install "${debfile}" && rm "${debfile}" && \
