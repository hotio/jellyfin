...
# install app
RUN debfile="/tmp/jellyfin.deb" && curl -fsSL -o "${debfile}" "https://repo.jellyfin.org/releases/server/ubuntu/unstable/jellyfin-server_${JELLYFIN_VERSION}-unstable_amd64.deb" && dpkg --install "${debfile}" && rm "${debfile}" && \
