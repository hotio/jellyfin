...
# install app
RUN debfile="/tmp/jellyfin.deb" && curl -fsSL -o "${debfile}" "https://repo.jellyfin.org/releases/server/ubuntu/nightly/jellyfin-server-nightly_${JELLYFIN_VERSION}_armhf.deb" && dpkg --install "${debfile}" && rm "${debfile}" && \
