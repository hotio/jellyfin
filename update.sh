...
EOF
    ...
    version=$(curl -fsSL "https://repo.jellyfin.org/releases/server/ubuntu/nightly/" | grep -o ">jellyfin-nightly_.*_amd64.deb<" | sed -e 's/>jellyfin-nightly_//g' -e 's/_amd64.deb<//g')
    [[ -z ${version} ]] && exit 1
    ...
    find . -type f -name '*.Dockerfile' -exec sed -i "s/ARG FFMPEG_VERSION=.*$/ARG FFMPEG_VERSION=${version_ffmpeg}/g" {} \;
