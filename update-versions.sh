#!/bin/bash
set -e
version=$(curl -fsSL "https://repo.jellyfin.org/ubuntu/dists/jammy/unstable/binary-amd64/Packages" | grep -A 7 -m 1 'Package: jellyfin-server')
version=$(awk -F ': ' '/Version/{print $2;exit}' <<< "${version}" | sed s/-unstable//g)
