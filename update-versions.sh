...
version=$(curl -fsSL "https://repo.jellyfin.org/ubuntu/dists/jammy/unstable/binary-amd64/Packages" | grep -A 7 -m 1 'Package: jellyfin-server' | awk -F ': ' '/Version/{print $2;exit}' | sed s/-unstable//g)
[[ -z ${version} ]] && exit 0
